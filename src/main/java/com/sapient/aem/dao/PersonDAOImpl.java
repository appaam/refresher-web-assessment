...
public class PersonDAOImpl implements PersonDAO  {
	...
	public Person getPersonByAadharCard(Integer aadhar_card) throws SQLException {
		...
		try  {
			...
			PreparedStatement preparedStatement= connection.prepareStatement(QueryMapper.GET_PERSON_BY_ADHARCARD);
	...
	public List<Person> getPerson() throws SQLException {
		...
		try {
			...
			ResultSet resultSet= statement.executeQuery(QueryMapper.GET_PERSON);
			int rowcount=0;
			if ( resultSet.last()) {
	...
	public Long addPerson(Person person) throws SQLException {
		...
		try{
			connection=Connections.jdbcConnection();
			PreparedStatement preparedStatement= connection.prepareStatement(QueryMapper.INSERT_PERSON);
	...
	public String updatePerson(Person person) throws SQLException {
		...
		try {
			connection=Connections.jdbcConnection();
			PreparedStatement preparedStatement= connection.prepareStatement(QueryMapper.UPDATE_PERSON);
	...
	public String deletePerson(Long adhar_card) throws SQLException {
		...
		try{
			connection=Connections.jdbcConnection();
			PreparedStatement preparedStatement= connection.prepareStatement(QueryMapper.DELETE_PERSON);
